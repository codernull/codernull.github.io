...
> 素材来自官方 Redis 仓库 `redis/redis`（本地 pinned tag `6.0.0` 与 `8.8.0` 走读）。所有源码片段标注文件、函数与 tag。
...
低延迟热路径上，Redis 放哪一层是决策，不是默认值。常见放法有三种：当缓存扛点查、当消息通道做事件分发、当状态源当主存储。这三种放法对“延迟”的承诺完全不同，而很多选型是用“Redis 快”这个模糊印象一起拍板的。

判断标准只有一条：**先问“哪个线程在扛”**。Redis 的吞吐和延迟特性不是由“快不快”决定的，而是由“命令执行只在一条线程上排队”决定的。这个约束没变过，那么所有基于“升级 Redis 就更快”的选型理由都要重新审。

本文的结论只在单机热路径层位成立：不覆盖集群多活、不覆盖大 Value 场景、不覆盖异步复制语义。

## 2. 单线程的真相：一个进程里其实有四种线程

Redis 的“单线程”特指**命令执行**，不是进程只有一条线程。从 2.4 起它就有后台线程了（`bio.c`），4.0 加了 lazyfree 异步释放，6.0 又多了 IO 线程。
...
三类任务：关闭文件、AOF fsync、lazyfree 内存释放。这三个共同点是**慢 I/O 或慢释放**——如果放主线程，一次慢 fsync 就能卡住所有命令。BIO 线程把它们搬走，但注意边界：**BIO 里没有任何数据操作**，命令读写不经过它们。
...
6.0 引入 IO 线程，动机是网卡变快后网络读写成为新瓶颈。关键在“搬了什么”：**读写搬走了，执行没搬**。
...
`writeToClient` 把回复写回客户端 socket，`readQueryFromClient` 把请求读进缓冲区。**两者都不执行命令**。命令执行发生在哪？在 `handleClientsWithPendingReadsUsingThreads` 的收尾——IO 线程把读完成之后，主线程重新跑一遍客户端列表（`networking.c`，tag `6.0.0`）：
...
注释已经写明白了：IO 线程把数据放进缓冲区，主线程“再跑一遍”去 `processCommandAndResetClient` + `processInputBuffer`——**执行永远回主线程**。`processCommandAndResetClient` 内部就是 `processCommand`（命令分发、ACL 检查、执行、写回复队列），全部在主线程。

还有两个配置细节，同样是边界的证据：

- `postponeClientRead`（`networking.c`，tag `6.0.0`）只有在 `server.io_threads_do_reads` 为真时才把读请求推迟给 IO 线程——而 `io-threads-do-reads` 默认是 `no`，即**默认读都不搬，只有写回走 IO 线程**。
- `stopThreadedIOIfNeeded`（`networking.c`，tag `6.0.0`）在 `pending < io_threads_num*2` 时停掉 IO 线程回到同步写——**低负载自动回退单线程**。
...
结构变了，注释把不变量写得很直白（`iothread.c`，tag `8.8.0`）：
...
“读进来、在主线程执行”——和 6.0 是同一句话。核对结论：**函数搬了家、实现重写了，命令执行仍在主线程这条不变量，跨 6.0→8.8 没动**。

## 4. 压测：瓶颈有没有搬家

正文高潮不是“开 4 个 IO 线程 GET 变快了多少”，而是验证一件事：**IO 线程到底把瓶颈搬走没有**。实验在 WSL + localhost 上跑，Redis 按 tag `6.0.0` 现编（MALLOC=libc），三档配置：`io-threads 1`（即关闭）、`io-threads 4`、`io-threads 4 + io-threads-do-reads yes`。
...
三档几乎一致（35–38 us）。这直接对应源码：单请求的路径是“读 → 主线程执行 → 写回”，IO 线程不改变这条路径。
...
8.x 的变化很多，但都可以归进外围：模块内置（8.0 起 Search/JSON/TimeSeries 不再需要单独装模块）、新数据结构（vector set、8.8 的 Array）、性能改进（延迟降低、内存压缩）——**没有一条改变“命令执行单线程”这条不变量**。本文只核对与主线相关的两点：
...
这次走读把“Redis 快”拆成了“哪种快”：点查快、状态快照快；但事件分发、高并发写回并不快——因为命令执行只有一条线程。对我来说，这足够回答最初的问题：**热路径上 Redis 该不该出现、出现在哪一层。**

**相关阅读（站内规划）**
- 低延迟系统 · 撮合引擎核心：订单簿结构、撮合语义与并发读取
- 低延迟系统 · Aeron 选型实录（待写）
- 分布式系统 · 消息中间件选型：Redpanda 能替代什么（待写）
