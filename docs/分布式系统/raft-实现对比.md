...
被隔离的节点只是暂时与集群失去联系，它本身依然是集群配置中的一员（除非显式通过配置变更移除它）。在标准的 Raft ，分布式集群中，无论是领导者真的故障，还是追随者因网络隔离收不到心跳，选举超时都是触发领导者更替的必经步骤：真正的领导者失联会被淘汰，集群通过新一轮选举产生新领导者；而仅仅是某个追随者自己网络不通时，它发起的选举会因为无法获得多数票而失败，已存在的合法领导者继续服务，集群不受影响。这样的节点会因收不到心跳而超时，进而递增自己的 Term 并发起选举；网络恢复后，它携带的更高 Term 会迫使现任合法领导者下台，造成不必要的扰动。
...
MongoDB 的设计思路与 etcd 类似：etcd 使用 PreVote 预投票机制，MongoDB 则通过dey-run机制在调度层发起选举。且实现位置不同——etcd 的 PreVote 在追随者节点内部触发，而 MongoDB 的选举决策位于心跳触发的决策层，决策层实现时区分不同的心跳触发场景来达到类似的控制效果，共有四种触发场景：
...
这种 dry-run 机制,分两层，调度决策层在 `ElectionState::start()`（`replication_coordinator_impl_elect_v1.cpp`，协调/调度层），实现在`TopologyCoordinator::processReplSetRequestVotes`（决策层） `TopologyCoordinator` 集群拓扑协调器中，是一次应用层的预演。与 PreVote 一样，它能有效减少因网络隔离导致的无谓任期增长、CPU 浪费和脑裂风险。
...
Redis Cluster 的故障转移也有类似特点。从节点发起选举时，会比较 configEpoch，同样没有 Raft 那样严格的日志完整性限制（即不强制要求候选者的日志至少和投票者一样新）。理论上，如果仅看投票条件，似乎可能选出一个数据落后的从节点，从而导致已确认的写入在后续被覆盖。但实际执行中，Cluster 在选择新主时，会考察候选从节点的状态及其与故障主节点的数据延迟，天然会倾向于数据更新、更健康的节点；同时，Redis 的异步复制本身允许少量数据丢失，并不会出现“选出一个落后节点覆盖掉多数节点已确认数据”的脑裂式覆盖。见第六节。
