...
被隔离的节点只是暂时与集群失去联系，它本身依然是集群配置中的一员（除非显式通过配置变更移除它）。在标准的 Raft ，分布式集群中，无论是领导者真的故障，还是追随者因网络隔离收不到心跳，选举超时都是触发领导者更替的必经步骤：真正的领导者失联会被淘汰，集群通过新一轮选举产生新领导者；而仅仅是某个追随者自己网络不通，等到网络恢复后，它携带的更高 Term 会迫使现任合法领导者下台，集群产生脑裂。原因是这样的节点会因收不到心跳而超时，进而递增自己的 Term 并发起选举；
...
这种 dry-run 机制,分两层，调度决策层在 `ElectionState::start()`（`replication_coordinator_impl_elect_v1.cpp`，协调/调度层），实现是在`TopologyCoordinator::processReplSetRequestVotes`（决策层） 集群拓扑协调器中，是一次应用层的预演。与 PreVote 一样，它能有效减少因网络隔离导致的无谓任期增长、CPU 浪费和脑裂风险。
